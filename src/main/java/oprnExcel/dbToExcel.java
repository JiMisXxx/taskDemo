...
public class dbToExcel implements Runnable {
    ...
    String filename = null;//文件名，若为空则使用查询条件作为文件名
        static String str = "select * from menzhenRC221227 where `\uFEFF机构编码` = ?;";
    ...
    public void writeinto(String conditions) throws Exception {
        ...
        System.out.println("文件[" + filename + ".xlsx]正在获取数据，时间：" + LocalDateTime.now().format(df));
        ...
        System.out.println("文件[" + filename + ".xlsx]获取数据完成，开始写入数据，时间：" + LocalDateTime.now().format(df));
